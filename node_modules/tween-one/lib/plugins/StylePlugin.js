...
var style = /*#__PURE__*/(0, _createClass2.default)( //CSSStyleDeclaration;
function style(_props) {
  ...
  this.setStartAtValue = function (start, startAt) {
    return (0, _typeof2.default)(startAt) === 'object' ? (0, _utils.getStartAtValue)(start, startAt) : start;
  };
  ...
  this.getAnimStart = function () {
    ...
    for (var key in propsData.data) {
      ...
      if (cssName === 'transform') {
        transform = transform || (startData === varsValue ? (0, _objectSpread2.default)({}, varsValue) : (0, _styleUtils.getTransform)(_this.computedStyle[_this.transformName]));
        var startValue = transform[key];

        if (startAt && key in startAt) {
          var _varsValue$key, _varsValue$key2;

          startValue = _this.setStartAtValue(startAt[key].unit && startAt[key].unit !== 'px' ? (0, _utils.startConvertToEndUnit)({
            target: target,
        ...
        if (startUnit !== endUnit) {
          transform[key] = (0, _utils.startConvertToEndUnit)({
            target: target,
            computedStyle: _this.computedStyle,
            style: cssName,
            value: startValue,
        ...
        filter = filter || (startData === varsValue ? (0, _objectSpread2.default)({}, varsValue) : (0, _styleUtils.splitFilterToObject)(styleValue) || {});
        filter[fKey] = filter[fKey] || _utils.initFilterValue[fKey];
        ...
        startUnit = (0, _typeof2.default)(startData) === 'object' ? startData.unit : "".concat(startData).replace(/[^a-z|%]/g, '');
        ...
        start[cssName] = parseFloat(startData) || (0, _utils.getDefaultStart)(cssName);
      }
    }
  ...
  this.render = function (ratio) {
    ...
    if (start[_this.transformName] && !tweenVars[_this.transformName]) {
      tweenVars[_this.transformName] = (0, _objectSpread2.default)({}, start[_this.transformName]);
    }

    if (start[_this.filterName] && !tweenVars[_this.filterName]) {
      tweenVars[_this.filterName] = (0, _objectSpread2.default)({}, start[_this.filterName]);
    }
    ...
    for (var key in data) {
      ...
      if (isT) {
        if (key === 'scale') {
          ...
          transform[key] = (0, _utils.getValue)(parseFloat(startValue), endValue, ratio, count, unit);
        }

        transform.text = (0, _utils.getTransformValue)(tweenVars[_this.transformName]);
      } else if (_styleUtils.cssList.filter.indexOf(key) >= 0) {
        var fKey = _styleUtils.cssList.filterConvert[key] || key;
        startValue = parseFloat(start[_this.filterName][fKey]) || 0;
        filter[fKey] = (0, _utils.getValue)(startValue, endValue, ratio, count, unit);
        ...
        if (typeof endValue === 'string') {
          ...
          tweenVars[key] = (0, _utils.getValue)(startValue, endValue, ratio, count, unit);
        }
      }
    }
